...
public final class ReflowMenuItemView extends MaterialCardView implements MenuView.ItemView {
    ...
    public ReflowMenuItemView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        iconContainer = findViewById(R.id.item_icon_container);
        activeIndicatorView = findViewById(R.id.item_active_indicator_view);
        icon = findViewById(R.id.item_icon);
        labelGroup = findViewById(R.id.item_labels_group);
        smallLabel = findViewById(R.id.item_small_label_view);
        largeLabel = findViewById(R.id.item_large_label_view);
        ...
        // the content description of the ReflowMenuItemView should be used for accessibility.
        ...
        if (icon != null) {
            icon.addOnLayoutChangeListener((v, left, top, right, bottom, oldLeft, oldTop, oldRight, oldBottom) -> {
    ...
    public void setLabelVisibilityMode(@ReflowMenuViewWrapper.LabelVisibility int mode) {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        final int width = w;
        post(() -> updateActiveIndicatorAvailableWidth(width));
    }
     ...
     */
    private void setActiveIndicatorProgress(@FloatRange(from = 0F, to = 1F) float progress, float target) {
     ...
     */
    private void maybeAnimateActiveIndicatorToProgress(@FloatRange(from = 0F, to = 1F) final float newProgress) {
    ...
    public void setChecked(boolean checked) {
        largeLabel.setPivotX(largeLabel.getWidth() / 2F);
        largeLabel.setPivotY(largeLabel.getBaseline());
        smallLabel.setPivotX(smallLabel.getWidth() / 2F);
        ...
        switch (labelVisibilityMode) {
            case ReflowMenuViewWrapper.LABEL_VISIBILITY_AUTO:
                if (isShifting) {
                    if (checked) {
            ...
            case ReflowMenuViewWrapper.LABEL_VISIBILITY_SELECTED:
                if (checked) {
                    // Show icon and large label
                    setViewTopMarginAndGravity(
                            getIconOrContainer(),
            ...
            case ReflowMenuViewWrapper.LABEL_VISIBILITY_LABELED:
                updateViewPaddingBottom(labelGroup, itemPaddingBottom);
                if (checked) {
                    // Show icon and large label
                    setViewTopMarginAndGravity(
            ...
            case ReflowMenuViewWrapper.LABEL_VISIBILITY_UNLABELED:
                // Show icon only
                setViewTopMarginAndGravity(getIconOrContainer(), itemPaddingTop, Gravity.CENTER);
    ...
    private void updateActiveIndicatorAvailableWidth(int availableWidth) {
        ...
        int newWidth = min(activeIndicatorDesiredWidth, availableWidth - (activeIndicatorMarginHorizontal * 2));
        FrameLayout.LayoutParams indicatorParams = (FrameLayout.LayoutParams) activeIndicatorView.getLayoutParams();
    ...
    private int getSuggestedIconWidth() {
        int badgeWidth = badgeDrawable == null
                ? 0
