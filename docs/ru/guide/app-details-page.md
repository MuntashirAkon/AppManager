...
Страница **«Сведения о приложении»** состоит из 11 (одиннадцати) вкладок. В основном она описывает почти каждый бит информации, которую может иметь приложение, включая все атрибуты из манифеста приложения, разрешения, [операции приложения][1], информацию о подписи и т. д.
...
- **Горизонтальная панель действий** это панель действий, содержащая различные действия в приложении. Смотрите [ниже](#горизонтальная-панель-действий) полный список доступных действий.
- **Пути и каталоги.** Содержит различную информацию о путях приложений, включая _каталог приложения_ (где хранятся файлы APK), _каталог данных_ (внутренние, защищенные устройством и внешние), _каталог разделенного APK_ (вместе с именами частей), и _нативную библиотеку JNI_ (если есть). Библиотеки JNI используются для вызова собственных кодов, обычно написанных на языке C/C ++. Использование собственной библиотеки может ускорить работу приложения или помочь приложению использовать сторонние библиотеки, написанные на языках, отличных от Java, как в большинстве игр. Вы также можете открывать эти каталоги с помощью ваших файловых менеджеров (при условии, что они их поддерживают и имеют необходимые разрешения), нажав на значок запуска с правой стороны каждого элемента.
- **Использование данных с последнего запуска устройства.** Опция, которая не требует пояснений. Но имейте в виду, что из-за некоторых проблем результаты часто могут вводить в заблуждение и быть просто неверными. Эта часть остается скрытой, если не предоставить разрешение на _доступ к истории использования_ на новых устройствах.
- **Хранилище и кэш.** Отображает информацию о размере приложения (файлы APK), данных и кэша. В старых устройствах также отображается размер папок с внешними данными, кэшем, медиа и obb. Эта часть остается скрытой, если не представить разрешение _доступ к истории использования_ на новых устройствах.
- **Подробнее.** Отображает прочую информацию, такую как
  ...
  * **Общий идентификатор пользователя.** Применим для приложений, которые используются вместе. Хотя и написано, что это идентификатор, на самом деле это строковое значение. Общее приложение должно иметь такие же [подписи](#вкладка-«подписи»).
...
- **Отключить.** Отключение приложения. Эта кнопка не отображается для уже отключенных приложений или для пользователей, у которых нет root-прав или [ADB][2]. Если вы отключите приложение, оно не будет отображаться в вашем лончере. Ярлыки приложения также будут удалены. Если вы отключите пользовательское приложение, вы можете включить их только через App Manager или любое другое приложение, которое поддерживает такую функцию. В настройках Android нет возможности включить отключенное пользовательское приложение.
...
- **Остановить принудительно.** Принудительная остановка приложения. Если вы принудительно остановите приложение, оно не будет работать в фоновом режиме, до того, как вы не откроете его повторно. Однако это не всегда так.
- **Очистить данные.** Очистка данных приложения. В данные приложения входит любая информация, хранящаяся во внутреннем и внешнем каталоге, включая учетные записи (если они настроены в приложении), кэш и т. д. Например, при очистке данных из App Manager удаляются все правила (однако блокировка не снимается), сохраненные в приложении. Вот почему вы всегда должны делать резервные копии своих правил. Эта кнопка не отображается для пользователей, у которых нет root-прав или [ADB][2].
- **Очистить кэш.** Очистка только кэш приложения. В настройках Android нет возможности очистить кэш приложения. Следовательно, для очистки кэша из внутренней памяти приложения требуется root.
- **Установить.** Установка APK, открытого с помощью любого стороннего приложения. Эта кнопка отображается только для внешних файлов APK, которые еще не были установлены.
- **Что нового.** Эта кнопка отображается для файла APK с более высоким кодом версии, чем установленный. При нажатии на эту кнопку отображается диалоговое окно с описанием различий в способах управления версиями. Информация, которую он отображает, включает в себя: _версию_, _трекеры_, _разрешения_, _компоненты_, _подписи_ (изменения контрольной суммы), _функции_, _общие библиотеки_ и _SDK_.
- **Обновить.** Отображается для приложений с более высоким кодом версии, чем установленная.
...
- **Манифест.** При нажатии на эту кнопку на отдельной странице показывается файл манифеста приложения. Файл манифеста можно свернуть или развернуть с помощью соответствующей кнопки-переключателя (в правом верхнем углу) или сохранить в общем хранилище с помощью кнопки сохранения.
- **Сканер.** Нажатие на эту кнопку отображает информацию о трекерах и библиотеках приложения. Сначала приложение сканируется для извлечения списка классов. Затем список классов сопоставляется с числом подписей. После этого отображается сводка сканирования. _Смотрите также: [Страница сканера][scanner]_
...
- **Поделиться.** Кнопка Поделиться может использоваться, чтобы поделиться файлом APK или _APKS_, (если приложение разделено на несколько частей) который можно импортировать в [SAI][sai]. Вы можете поделиться им с помощью вашего любимого файлового менеджера, чтобы сохранить файл в общем хранилище.
- **Обновить.** Обновляет вкладку «О приложении».
- **Показать в настройках.** Открывает приложение в настройках Android.
- **Резервное копирование/восстановление.** Открывает диалоговое окно резервного копирования/восстановления.
- **Экспортировать правила блокировки.** Экспортирует правила, настроенные для этого приложения с помощью App Manager.
...
Вы также можете создать ярлык для этих _экспортируемых_ активити (используя специальную кнопку), и при желании вы также можете редактировать ярлык, используя кнопку _изменить ярлык_.
...
Когда вы закрываете активити, оно обычно закрывается моментально (в зависимости от многих факторов, таких как объем свободной памяти вашего телефона). В то время как службы могут работать неопределенный период. Если в фоновом режиме работает больше служб, ваш телефон будет работать медленнее из-за нехватки памяти и/или вычислительной мощности, а аккумулятор вашего телефона может разряжаться быстрее. В более новых версиях Android для всех приложений по умолчанию включена функция экономии заряда батареи. Если эта функция включена, система может случайным образом прекратить работу любой службы.

Кстати, и активити, и службы выполняются в одном и том же лупере, который называется основным [лупером][looper], что означает, что службы на самом деле не работают в фоновом режиме. Авторы приложения работают над исправлением этого. Как приложения взаимодействуют со службами? Они используют [широковещательные приемники](#приемники).

### Приемники
**Приемники** (также известные как _широковещательные приемники_) могут использоваться для запуска выполнения определенных задач для определенных событий. Эти компоненты называются широковещательными приемниками, потому что они выполняются сразу после получения широковещательного сообщения. Эти широковещательные сообщения отправляются с использованием метода, называемого намерением. Намерение (intent) – это специальная функция Android, которую можно использовать для открытия приложений, активити, служб и отправки широковещательных сообщений. Поэтому, так же, как и [активити](#активити), широковещательные приемники используют фильтры намерений, чтобы получать только желаемые широковещательное сообщения. Широковещательные сообщения могут быть отправлены системой или самим приложением. Когда отправляется широковещательное сообщение, соответствующие приемники активируются системой, чтобы они могли выполнять задачи. Например, если у вас мало памяти, ваш телефон может тормозить или зависать на мгновение после того, как вы включите мобильные данные или подключитесь к Wi-Fi. Вы когда-нибудь задумывались, почему? Это потому, что широковещательные приемники, которые могут принимать `android.net.conn.CONNECTIVITY_CHANGE` просыпаются системой, как только вы включаете передачу данных. Поскольку многие приложения используют этот фильтр намерений, все эти приложения почти сразу же пробуждаются системой, что вызывает зависание или задержки. При этом приемники могут использоваться для межпроцессного взаимодействия (IPC), т.е. они помогают вам общаться между разными приложениями (при условии, что у вас есть необходимые разрешения) или даже между различными компонентами одного приложения.
...
- **Режим.** Он описывает текущий статус авторизации, значение которого может быть _allow (разрешить)_, _deny (отклонить)_ (если точнее, это просто означает ошибку), _ignore (игнорировать)_ (на самом деле означает отклонить/deny), _default (по умолчанию)_ (выводится из списка значений по умолчанию, установленных внутри оболочки устройства), _foreground (передний план)_ (в более новых устройствах Android это означает, что операция приложения может использоваться только тогда, когда приложение работает на переднем плане). Так же существуют другие пользовательские режимы, установленные поставщиками (MIUI использует _ask (спросить)_ и другие режимы только с числами, без каких-либо связанных имен).
...
Рядом с каждым элементом операции приложения есть кнопка-переключатель, с помощью которой можно разрешить или запретить (игнорировать) операцию приложения. Вы также можете сбросить свои изменения, используя опцию _сброс по умолчанию_ или отклонить все опасные операции приложения, используя соответствующую опцию в меню. Вы также можете сортировать их в порядке возрастания по названиям опций приложений и связанным уникальным числам (или значениям). Вы также можете сначала вывести список запрещенных операций приложения, используя соответствующий параметр сортировки.

::: warning Предупреждение
Отклонение операции приложения может привести к некорректному функционированию приложения. В этом случае используйте опцию _сброс по умолчанию_.
:::
...
## Вкладка «Подписи»
**Подписи** на самом деле называется информацией о подписи. Приложение подписывается разработчиками приложения, одним или несколькими сертификатами перед его публикацией. Целостность приложения (независимо от того, принадлежит ли оно фактическому разработчику и не изменено ли оно другими людьми) можно проверить с помощью информации о подписи; потому что, когда приложение изменено сторонним неавторизованным лицом, приложение не может быть снова подписано с использованием исходного сертификата(ов), потому что информация о подписи хранится в секрете фактическим разработчиком. _Как вы верифицируете эти подписи?_ С помощью контрольных сумм. Контрольные суммы создаются из самих сертификатов. Если разработчик предоставляет контрольные суммы, вы можете сопоставить контрольные суммы, используя разные контрольные суммы, сгенерированные во вкладке **подписи**. Например, если вы загрузили App Manager с GitHub, канала Telegram или репозитория IzzyOnDroid, вы можете проверить, действительно ли приложение выпущено мной, просто сопоставив следующую контрольную сумму _sha256_ с тем, что отображается в этой вкладке:
```
...
[settings_gcb]: ./settings-page.md#глобальная-блокировка-компонентов
