...
Как описано в предыдущем разделе, операции, настроенные для приложения, хранятся в [/data/system/appops.xml](#appops-xml). Если операция не настроена, то будет ли система разрешать эту операцию, определяется из [`sOpDefaultMode`][9]. В нем отображается _режим по умолчанию_ для каждой операции.
...
/**
 * the given caller should use its default security check. Этот режим обычно не используется
 */
...
Последний `appops.xml` имеет следующий формат: (это DTD создано мной и никоим образом не идеально, имеет проблемы совместимости).
